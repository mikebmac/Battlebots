...
const static int IN2 = 5;
const static int IN3 = 4;
const static int ENB = A0;
const static int IN4 = 8; // was 2
  ...
  if (_dataWasReceived && !safetyOn)
  {
    ...
    while (_radio.hasDataISR()) {
     ...
     int joyYmap;
      ...
      if (_radioData1.joyY < 105) {
        digitalWrite(IN1, LOW);
        digitalWrite(IN2, HIGH);
        joyYmap = map(_radioData1.joyY, 0, 105, 255, 0);
      } else if (_radioData1.joyY > 120) {
        digitalWrite(IN1, HIGH);
        digitalWrite(IN2, LOW);
        joyYmap = map(_radioData1.joyY, 120, 225, 0, 255);
      } else {
        joyYmap = 0;
      }
      ...
      if (_radioData1.joyX < 105) {
        digitalWrite(IN3, HIGH);
        digitalWrite(IN4, LOW);
        joyXmap = map(_radioData1.joyX, 0, 105, 255, 0);
      } else if (_radioData1.joyX > 120) {
        digitalWrite(IN3, LOW);
        digitalWrite(IN4, HIGH);
        joyXmap = map(_radioData1.joyX, 120, 225, 0, 255);
      } else {
        joyXmap = 0;
      }
